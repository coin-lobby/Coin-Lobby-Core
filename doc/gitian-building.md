...
*Setup instructions for a Gitian build of CoinLobby Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CoinLobby
...
- [Building CoinLobby Core](#building-coinlobby-core)
...
Clone the git repositories for CoinLobby Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/coin-lobby/coinlobby
git clone https://github.com/coin-lobby/gitian.sigs.git
```
...
in the CoinLobby Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building CoinLobby Core
----------------

To build CoinLobby Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the CoinLobby Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/coinlobby/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/coin-lobby/coinlobby
...
For example:
```bash
URL=https://github.com/crowning-/coinlobby.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit coinlobby=${COMMIT} --url coinlobby=${URL} ../coinlobby/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit coinlobby=${COMMIT} --url coinlobby=${URL} ../coinlobby/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit coinlobby=${COMMIT} --url coinlobby=${URL} ../coinlobby/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the coinlobby git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../coinlobby/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/coin-lobby/coinlobby-detached-sigs.git

BTCPATH=/some/root/path/coinlobby
SIGPATH=/some/root/path/coinlobby-detached-sigs

./bin/gbuild --url coinlobby=${BTCPATH},signature=${SIGPATH} ../coinlobby/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/coinlobby-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/coinlobby-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/coinlobby-osx-build.assert
```
...
[coinlobby/gitian.sigs](https://github.com/coin-lobby/gitian.sigs/) repository, or if that's not possible to create a pull
request.
