...
// Copyright (c) 2017-2018 The CNL Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "netbase.h"
#include "test/test_coinlobby.h"
...
BOOST_AUTO_TEST_CASE(netbase_splithost)
{
    ...
    BOOST_CHECK(TestSplitHost("127.0.0.1:7589", "127.0.0.1", 7589));
    BOOST_CHECK(TestSplitHost("[127.0.0.1]", "127.0.0.1", -1));
    BOOST_CHECK(TestSplitHost("[127.0.0.1]:7589", "127.0.0.1", 7589));
    BOOST_CHECK(TestSplitHost("::ffff:127.0.0.1", "::ffff:127.0.0.1", -1));
    BOOST_CHECK(TestSplitHost("[::ffff:127.0.0.1]:7589", "::ffff:127.0.0.1", 7589));
    BOOST_CHECK(TestSplitHost("[::]:7589", "::", 7589));
    BOOST_CHECK(TestSplitHost("::7589", "::7589", -1));
    BOOST_CHECK(TestSplitHost(":7589", "", 7589));
    BOOST_CHECK(TestSplitHost("[]:7589", "", 7589));
...
BOOST_AUTO_TEST_CASE(netbase_lookupnumeric)
{
    ...
    BOOST_CHECK(TestParse("127.0.0.1:7589", "127.0.0.1:7589"));
    BOOST_CHECK(TestParse("::ffff:127.0.0.1", "127.0.0.1:65535"));
    BOOST_CHECK(TestParse("::", "[::]:65535"));
    BOOST_CHECK(TestParse("[::]:7589", "[::]:7589"));
    BOOST_CHECK(TestParse("[127.0.0.1]", "127.0.0.1:65535"));
