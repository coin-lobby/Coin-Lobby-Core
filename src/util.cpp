...
const char * const BITCOIN_CONF_FILENAME = "coinlobby.conf";
const char * const BITCOIN_PID_FILENAME = "coinlobbyd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "coinlobby" is a composite category enabling all CoinLobby-related debug output
                if(ptrCategory->count(std::string("coinlobby"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CoinLobbyCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CoinLobbyCore
    // Mac: ~/Library/Application Support/CoinLobbyCore
    // Unix: ~/.coinlobbycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CoinLobbyCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CoinLobbyCore";
#else
    // Unix
    return pathRet / ".coinlobbycore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty coinlobby.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override coinlobby.conf
