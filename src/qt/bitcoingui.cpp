...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a CoinLobby address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and coinlobby: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about CoinLobby Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your CoinLobby addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified CoinLobby addresses"));
    ...
    openAction->setStatusTip(tr("Open a coinlobby: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible CoinLobby command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to CoinLobby network", "", count));
    } else {
...
{
    QString strTitle = tr("CoinLobby Core"); // default title
    ...
    // Append title to "CoinLobby Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
