...
{
    return QCoreApplication::translate("coinlobby-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in coinlobby.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in coinlobby.qrc)
...
/** Class encapsulating CoinLobby Core startup and shutdown.
...
/** Main CoinLobby application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // coinlobby: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. CoinLobby Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
//    Q_INIT_RESOURCE(coinlobby_locale);
    ...
    /// 6. Determine availability of data directory and parse coinlobby.conf
    ...
    // coinlobby: links repeatedly have their payment requests routed to this process:
