...
// which are part of the CNL repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="coinlobby-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /coinlobby-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "coinlobby-coinlobby/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from coinlobby-coinlobby/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'coinlobby-coinlobby/development', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from coinlobby-coinlobby/development"
            }
          }

          if (hasCache) {
            sh "cd /coinlobby-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /coinlobby-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /coinlobby-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /coinlobby-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /coinlobby-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /coinlobby-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /coinlobby-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
