...
RUN groupadd -g ${GROUP_ID} coinlobby
RUN useradd -u ${USER_ID} -g coinlobby -s /bin/bash -m -d /coinlobby coinlobby
...
RUN mkdir /coinlobby-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /coinlobby-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /coinlobby-src
